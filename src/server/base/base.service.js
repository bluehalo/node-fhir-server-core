...
module.exports.batch = (req, res) => new Promise((resolve, reject) => {
    ...
    if (resourceType !== 'Bundle') {
        return reject(errors.internal(`Expected 'resourceType: Bundle'. Received 'resourceType: ${resourceType}'.`,
            baseVersion));
    }
    if (type.toLowerCase() !== 'batch') {
        return reject(errors.internal(`Expected 'type: batch'. Received 'type: ${type}'.`,
            baseVersion));
    }
...
//  Lo dejo listo para implementar a futuro
module.exports.transaction = (req, res) => new Promise((resolve, reject) => {
    console.log('entro a una transaction.....');
    ...
    if (type.toLowerCase() !== 'transaction') {
        return reject(errors.internal(`Expected 'type: transaction'. Received 'type: ${type}'.`,
            baseVersion));
    }
    let { protocol, baseUrl } = req;
