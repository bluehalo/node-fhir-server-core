...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/basic/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('basic'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/basic/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('basic'),
