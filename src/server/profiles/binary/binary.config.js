...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/binary/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('binary'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/binary/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('binary'),
