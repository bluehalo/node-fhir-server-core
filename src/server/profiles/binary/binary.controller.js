...
module.exports.create = function create({ profile, logger, app, config }) {
	...
	return (req, res, next) => {
		...
		if (Binary.resourceType !== resource_body.resourceType) {
			return next(
				errors.invalidParameter(
					`'resourceType' expected to have value of '${Binary.resourceType}', received '${resource_body.resourceType}'`,
					base_version,
				),
			);
		}
		...
		return service
			...
			.then(results =>
				responseUtils.handleCreateResponse(req, res, base_version, Binary.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
module.exports.update = function update({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleUpdateResponse(req, res, base_version, Binary.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
