...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/bundle/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('bundle'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/bundle/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('bundle'),
