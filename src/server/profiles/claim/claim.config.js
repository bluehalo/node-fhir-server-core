...
const resource_specific_args = require('./claim.arguments');
...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/claim/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('claim'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/claim/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('claim'),
