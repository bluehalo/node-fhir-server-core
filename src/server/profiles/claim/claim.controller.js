...
let getResourceConstructor = base_version => {
	return require(resolveFromVersion(base_version, 'Claim'));
};
...
module.exports.searchByVersionId = function searchByVersionId({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => responseUtils.handleSingleReadResponse(res, next, base_version, Claim, results, version_id))
			.catch(err => {
				logger.error(err);
...
/**
 * @description Controller to search claim
 */
module.exports.search = function search({ profile, logger, config, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleReadResponse(res, base_version, Claim, results, {
...
/**
 * @description Controller to searchById claim
 */
module.exports.searchById = function searchById({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => {
				responseUtils.handleSingleReadResponse(res, next, base_version, Claim, results);
			})
			.catch(err => {
				logger.error(err);
				next(errors.internal(err.message, base_version));
...
/**
 * @description Controller for creating a claim
 */
module.exports.create = function create({ profile, logger, app, config }) {
	...
	return (req, res, next) => {
		...
		let args = { base_version, resource: claim };
...
/**
 * @description Controller for updating/creating Claim. If Claim does not exist, it should be updated
 */
module.exports.update = function update({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		// Create a new claim resource and pass it to the service
		let claim = new Claim(resource_body);
		let args = { id, base_version, resource: claim };
		// Pass any new information to the underlying service
		return service
...
/**
 * @description Controller for deleting Claim resource.
 */
...
/**
 * @description Controller for getting the history of Claim resource.
 */
module.exports.history = function history({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleHistoryResponse(res, base_version, Claim, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for getting the history of Claim resource by ID.
 */
