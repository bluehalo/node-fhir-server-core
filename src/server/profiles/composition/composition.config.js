...
const resource_specific_args = require('./composition.arguments');
...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/composition/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('composition'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/composition/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('composition'),
