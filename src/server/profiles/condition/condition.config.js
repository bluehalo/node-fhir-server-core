...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/condition/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('condition'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/condition/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('condition'),
