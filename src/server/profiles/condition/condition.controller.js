...
module.exports.searchByVersionId = function searchByVersionId({profile, logger, app}) {
	...
	return (req, res, next) => {
		...
		return service.searchByVersionId(req.sanitized_args, logger)
			.then((results) =>
				responseUtils.handleSingleVReadResponse(res, next, base, Condition, results, version_id)
			)
			.catch((err) => {
				logger.error(err);
...
module.exports.searchById = function searchById({profile, logger, app}) {
	...
	return (req, res, next) => {
		...
		return service.searchById(req.sanitized_args, logger)
			.then((condition) => {
				let Resource = getResourceConstructor(base, condition.resourceType);
				responseUtils.handleSingleReadResponse(res, next, base, Resource, condition);
			})
			.catch((err) => {
				logger.error(err);
				next(errors.internal(err.message, base));
...
module.exports.create = function create({profile, logger, app}) {
	...
	return (req, res, next) => {
		...
		let condition = new Resource(resource_body);
