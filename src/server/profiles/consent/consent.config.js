...
const resource_specific_args = require('./consent.arguments');
...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/consent/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('consent'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/consent/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('consent'),
