...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/device/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('device'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/device/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('device'),
