...
let getResourceConstructor = base_version => {
	return require(resolveFromVersion(base_version, 'EligibilityRequest'));
};
...
module.exports.searchByVersionId = function searchByVersionId({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleSingleReadResponse(res, next, base_version, EligibilityRequest, results, version_id),
			)
			.catch(err => {
...
module.exports.search = function search({ profile, logger, config, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleReadResponse(res, base_version, EligibilityRequest, results, {
...
module.exports.searchById = function searchById({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => {
				responseUtils.handleSingleReadResponse(res, next, base_version, EligibilityRequest, results);
			})
			.catch(err => {
				next(errors.internal(err, base_version));
			});
...
module.exports.create = function create({ profile, logger, app, config }) {
	...
	return (req, res, next) => {
		...
		let args = { base_version, resource: eligibilityrequest };
		// Pass any new information to the underlying service
		return service
			.create(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleCreateResponse(req, res, base_version, EligibilityRequest.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for updating/creating EligibilityRequest. If EligibilityRequest does not exist, it should be updated
 */
module.exports.update = function update({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		// Create a new eligibilityrequest resource and pass it to the service
		let eligibilityrequest = new EligibilityRequest(resource_body);
		let args = { id, base_version, resource: eligibilityrequest };
		// Pass any new information to the underlying service
		return service
			.update(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleUpdateResponse(req, res, base_version, EligibilityRequest.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for deleting EligibilityRequest resource.
 */
...
module.exports.history = function history({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleHistoryResponse(res, base_version, EligibilityRequest, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
