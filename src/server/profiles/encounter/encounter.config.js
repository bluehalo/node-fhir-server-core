...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/encounter/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('encounter'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/encounter/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('encounter'),
