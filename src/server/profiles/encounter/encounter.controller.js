...
let getResourceConstructor = base_version => {
	return require(resolveFromVersion(base_version, 'Encounter'));
};
...
module.exports.searchByVersionId = function searchByVersionId({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => responseUtils.handleSingleReadResponse(res, next, base_version, Encounter, results, version_id))
			.catch(err => {
...
module.exports.search = function search({ profile, logger, config, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleReadResponse(res, base_version, Encounter, results, {
...
module.exports.searchById = function searchById({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => {
				responseUtils.handleSingleReadResponse(res, next, base_version, Encounter, results);
			})
			.catch(err => {
				next(errors.internal(err, base_version));
			});
...
module.exports.create = function create({ profile, logger, app, config }) {
	...
	return (req, res, next) => {
		...
		let args = { base_version, resource: encounter };
		// Pass any new information to the underlying service
		return service
			.create(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleCreateResponse(req, res, base_version, Encounter.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for updating/creating Encounter. If Encounter does not exist, it should be updated
 */
module.exports.update = function update({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleUpdateResponse(req, res, base_version, Encounter.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for deleting Encounter resource.
 */
...
module.exports.history = function history({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleHistoryResponse(res, base_version, Encounter, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
