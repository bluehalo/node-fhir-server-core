...
const resource_specific_args = require('./flag.arguments');
const controller = require('./flag.controller');

const validationUtils = require('../../utils/validation.utils');

let write_only_scopes = write_scopes('Flag');
let read_only_scopes = read_scopes('Flag');
...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/flag/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('flag'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/flag/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('flag'),
