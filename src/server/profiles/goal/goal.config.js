...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/goal/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('goal'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/goal/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('goal'),
