...
let getResourceConstructor = (base) => {
	return require(resolveFromVersion(base, 'Goal'));
};
...
module.exports.searchByVersionId = function searchByVersionId({profile, logger, app}) {
	...
	return (req, res, next) => {
		...
		let Goal = getResourceConstructor(base);

		return service.searchByVersionId(req.sanitized_args, req.contexts, logger)
			.then((results) =>
				responseUtils.handleSingleReadResponse(res, next, base, Goal, results, version_id)
			)
			.catch((err) => {
				logger.error(err);
...
module.exports.history = function history ({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service.history(req.sanitized_args, logger)
			.then((results) =>
				responseUtils.handleBundleHistoryResponse( res, base, Goal, results, {
					resourceUrl: config.auth.resourceServer
				})
			)
			.catch((err) => {
