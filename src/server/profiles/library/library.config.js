...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/library/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('library'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/library/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('library'),
