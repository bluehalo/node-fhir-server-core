...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/linkage/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('linkage'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/linkage/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('linkage'),
