...
const resource_specific_args = require('./list.arguments');
...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/list/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('list'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/list/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('list'),
