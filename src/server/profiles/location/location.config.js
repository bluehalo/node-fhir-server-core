...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/location/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('location'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/location/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('location'),
