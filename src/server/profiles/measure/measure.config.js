...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/measure/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('measure'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/measure/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('measure'),
