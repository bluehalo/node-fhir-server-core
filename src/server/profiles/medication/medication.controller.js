...
module.exports.searchByVersionId = function searchByVersionId({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => responseUtils.handleSingleReadResponse(res, next, base_version, Medication, results, version_id))
			.catch(err => {
...
module.exports.create = function create({ profile, logger, app, config }) {
	...
	return (req, res, next) => {
		...
		let args = { base_version, resource: medication };
		// Pass any new information to the underlying service
		return service
			.create(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleCreateResponse(req, res, base_version, Medication.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for updating/creating Medication. If Medication does not exist, it should be updated
 */
module.exports.update = function update({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		// Create a new medication resource and pass it to the service
		let medication = new Medication(resource_body);
		let args = { id, base_version, resource: medication };
		// Pass any new information to the underlying service
		return service
			.update(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleUpdateResponse(req, res, base_version, Medication.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for deleting Medication resource.
 */
