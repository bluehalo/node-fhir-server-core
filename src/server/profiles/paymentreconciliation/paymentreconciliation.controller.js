...
let getResourceConstructor = base_version => {
	return require(resolveFromVersion(base_version, 'PaymentReconciliation'));
};
...
module.exports.searchByVersionId = function searchByVersionId({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleSingleReadResponse(res, next, base_version, PaymentReconciliation, results, version_id),
			)
			.catch(err => {
...
/**
 * @description Controller to search paymentreconciliation
 */
...
/**
 * @description Controller to searchById paymentreconciliation
 */
...
/**
 * @description Controller for creating a paymentreconciliation
 */
module.exports.create = function create({ profile, logger, app, config }) {
	...
	return (req, res, next) => {
		...
		let args = { base_version, resource: paymentreconciliation };
		// Pass any new information to the underlying service
		return service
			.create(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleCreateResponse(req, res, base_version, PaymentReconciliation.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for updating/creating PaymentReconciliation. If PaymentReconciliation does not exist, it should be updated
 */
module.exports.update = function update({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleUpdateResponse(req, res, base_version, PaymentReconciliation.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for deleting PaymentReconciliation resource.
 */
...
/**
 * @description Controller for getting the history of PaymentReconciliation resource.
 */
module.exports.history = function history({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleHistoryResponse(res, base_version, PaymentReconciliation, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for getting the history of PaymentReconciliation resource by ID.
 */
