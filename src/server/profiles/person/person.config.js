...
const resource_specific_args = require('./person.arguments');
const controller = require('./person.controller');

const validationUtils = require('../../utils/validation.utils');

let write_only_scopes = write_scopes('Person');
let read_only_scopes = read_scopes('Person');
...
let routes = [
	...
	{
		type: 'post',
		path: '/:base/person/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('person'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/person/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('person'),
