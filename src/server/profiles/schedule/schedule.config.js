...
const resource_specific_args = require('./schedule.arguments');
const controller = require('./schedule.controller');

const validationUtils = require('../../utils/validation.utils');

let write_only_scopes = write_scopes('Schedule');
let read_only_scopes = read_scopes('Schedule');
...
let routes = [
	...
	{
		type: 'get',
		path: '/:base/schedule/:id/_history',
		args: resource_all_arguments,
		scopes: read_only_scopes,
	...
	{
		type: 'post',
		path: '/:base/schedule/([\$])validate',
		args: [
			route_args.BASE
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModel('schedule'),
		dependencies: []
	},
	{
		type: 'post',
		path: '/:base/schedule/:id/([\$])validate',
		args: [
			route_args.BASE,
			route_args.ID
		],
		scopes: read_only_scopes,
		controller: validationUtils.validateModelById('schedule'),
