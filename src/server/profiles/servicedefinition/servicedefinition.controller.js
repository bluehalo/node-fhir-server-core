...
let getResourceConstructor = base_version => {
	return require(resolveFromVersion(base_version, 'ServiceDefinition'));
};
...
module.exports.searchByVersionId = function searchByVersionId({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleSingleReadResponse(res, next, base_version, ServiceDefinition, results, version_id),
			)
			.catch(err => {
...
module.exports.search = function search({ profile, logger, config, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleReadResponse(res, base_version, ServiceDefinition, results, {
...
module.exports.searchById = function searchById({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => {
				responseUtils.handleSingleReadResponse(res, next, base_version, ServiceDefinition, results);
			})
			.catch(err => {
				next(errors.internal(err, base_version));
			});
...
module.exports.create = function create({ profile, logger, app, config }) {
	...
	return (req, res, next) => {
		...
		let args = { base_version, resource: servicedefinition };
		// Pass any new information to the underlying service
		return service
			.create(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleCreateResponse(req, res, base_version, ServiceDefinition.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
module.exports.update = function update({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		// Create a new servicedefinition resource and pass it to the service
		let servicedefinition = new ServiceDefinition(resource_body);
		let args = { id, base_version, resource: servicedefinition };
		// Pass any new information to the underlying service
		return service
			.update(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleUpdateResponse(req, res, base_version, ServiceDefinition.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
module.exports.history = function history({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results =>
				responseUtils.handleBundleHistoryResponse(res, base_version, ServiceDefinition, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
