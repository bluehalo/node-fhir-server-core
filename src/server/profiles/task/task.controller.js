...
let getResourceConstructor = (base_version) => {
	return require(resolveFromVersion(base_version, 'Task'));
};
...
module.exports.searchByVersionId = function searchByVersionId({profile, logger, app}) {
	...
	return (req, res, next) => {
		...
		return service.searchByVersionId(req.sanitized_args, req.contexts, logger)
			.then((results) =>
				responseUtils.handleSingleReadResponse(res, next, base_version, Task, results, version_id)
			)
			.catch((err) => {
				logger.error(err);
...
module.exports.search = function search({profile, logger, config, app}) {
	...
	return (req, res, next) => {
		...
		return service.search(req.sanitized_args, req.contexts, logger)
			.then((results) =>
				responseUtils.handleBundleReadResponse(res, base_version, Task, results, {
...
module.exports.searchById = function searchById({profile, logger, app}) {
	...
	return (req, res, next) => {
		...
		return service.searchById(req.sanitized_args, req.contexts, logger)
			.then((results) => {
				responseUtils.handleSingleReadResponse(res, next, base_version, Task, results);
			})
			.catch((err) => {
				logger.error(err);
				next(errors.internal(err.message, base_version));
...
module.exports.create = function create({profile, logger, app}) {
	...
	return (req, res, next) => {
		...
		let args = {id: resource_id, base_version, resource: task};
...
/**
 * @description Controller for updating/creating Task. If Task does not exist, it should be updated
 */
module.exports.update = function update ({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		// Create a new task resource and pass it to the service
		let task = new Task(resource_body);
		let args = {id, base_version, resource: task};
		// Pass any new information to the underlying service
...
/**
 * @description Controller for deleting Task resource.
 */
...
module.exports.history = function history ({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		return service.history(req.sanitized_args, req.contexts, logger)
			.then((results) =>
				responseUtils.handleBundleHistoryResponse( res, base_version, Task, results, {
					resourceUrl: config.auth.resourceServer
				})
			)
			.catch((err) => {
