...
let getResourceConstructor = base_version => {
	return require(resolveFromVersion(base_version, 'ValueSet'));
};
...
module.exports.searchByVersionId = function searchByVersionId({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => responseUtils.handleSingleReadResponse(res, next, base_version, ValueSet, results, version_id))
			.catch(err => {
...
/**
 * @description Controller to search valueset
 */
...
/**
 * @description Controller to searchById valueset
 */
module.exports.searchById = function searchById({ profile, logger, app }) {
	...
	return (req, res, next) => {
		...
		return service
			...
			.then(results => {
				responseUtils.handleSingleReadResponse(res, next, base_version, ValueSet, results);
			})
			.catch(err => {
				next(errors.internal(err, base_version));
			});
...
/**
 * @description Controller for creating a valueset
 */
module.exports.create = function create({ profile, logger, app, config }) {
	...
	return (req, res, next) => {
		...
		let valueset = new ValueSet(resource_body);
		let args = { base_version, resource: valueset };
		// Pass any new information to the underlying service
		return service
			.create(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleCreateResponse(req, res, base_version, ValueSet.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for updating/creating ValueSet. If ValueSet does not exist, it should be updated
 */
module.exports.update = function update({ profile, logger, config }) {
	...
	return (req, res, next) => {
		...
		// Create a new valueset resource and pass it to the service
		let valueset = new ValueSet(resource_body);
		let args = { id, base_version, resource: valueset };
		// Pass any new information to the underlying service
		return service
			.update(args, req.contexts, logger)
			.then(results =>
				responseUtils.handleUpdateResponse(req, res, base_version, ValueSet.resourceType, results, {
					resourceUrl: config.auth.resourceServer,
				}),
			)
			.catch(err => {
...
/**
 * @description Controller for deleting ValueSet resource.
 */
...
/**
 * @description Controller for getting the history of ValueSet resource.
 */
...
/**
 * @description Controller for getting the history of ValueSet resource by ID.
 */
...
/**
 * @description Controller for expanding a ValueSet by ID.
 */
module.exports.expandById = function expandById({ profile, logger, config }) {
	let { serviceModule: service } = profile;
