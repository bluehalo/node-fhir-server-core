...
class AuditEvent extends DomainResource {
	...
	set participant (new_value) {
		...
		this.__participant = Array.isArray(new_value) ? new_value.map(val => new AuditEventParticipant(val)) : [new AuditEventParticipant(new_value)];
	}
	...
	set object (new_value) {
		...
		this.__object = Array.isArray(new_value) ? new_value.map(val => new AuditEventObject(val)) : [new AuditEventObject(new_value)];
	}
