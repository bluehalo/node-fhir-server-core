...
class CarePlan extends DomainResource {
	...
	set relatedPlan (new_value) {
		...
		this.__relatedPlan = Array.isArray(new_value) ? new_value.map(val => new CarePlanRelatedPlan(val)) : [new CarePlanRelatedPlan(new_value)];
	}
	...
	set participant (new_value) {
		...
		this.__participant = Array.isArray(new_value) ? new_value.map(val => new CarePlanParticipant(val)) : [new CarePlanParticipant(new_value)];
	}
