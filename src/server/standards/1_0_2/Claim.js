...
class Claim extends DomainResource {
	...
	set coverage (new_value) {
		...
		this.__coverage = Array.isArray(new_value) ? new_value.map(val => new ClaimCoverage(val)) : [new ClaimCoverage(new_value)];
	}
	...
	set interventionException (new_value) {
		const Coding = require('./Coding');
		this.__interventionException = Array.isArray(new_value) ? new_value.map(val => new Coding(val)) : [new Coding(new_value)];
	}
	...
	set additionalMaterials (new_value) {
		const Coding = require('./Coding');
		this.__additionalMaterials = Array.isArray(new_value) ? new_value.map(val => new Coding(val)) : [new Coding(new_value)];
	}
	...
	set missingTeeth (new_value) {
		...
		this.__missingTeeth = Array.isArray(new_value) ? new_value.map(val => new ClaimMissingTeeth(val)) : [new ClaimMissingTeeth(new_value)];
	}
