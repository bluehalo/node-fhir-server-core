...
class ClinicalImpression extends DomainResource {
	...
	set investigations (new_value) {
		...
		this.__investigations = Array.isArray(new_value) ? new_value.map(val => new ClinicalImpressionInvestigations(val)) : [new ClinicalImpressionInvestigations(new_value)];
	}
	...
	set resolved (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__resolved = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set ruledOut (new_value) {
		...
		this.__ruledOut = Array.isArray(new_value) ? new_value.map(val => new ClinicalImpressionRuledOut(val)) : [new ClinicalImpressionRuledOut(new_value)];
	}
