...
class ConformanceMessaging extends BackboneElement {
	...
	set endpoint (new_value) {
		...
		this.__endpoint = Array.isArray(new_value) ? new_value.map(val => new ConformanceMessagingEndpoint(val)) : [new ConformanceMessagingEndpoint(new_value)];
	}
	...
	set event (new_value) {
		...
		this.__event = Array.isArray(new_value) ? new_value.map(val => new ConformanceMessagingEvent(val)) : [new ConformanceMessagingEvent(new_value)];
	}
