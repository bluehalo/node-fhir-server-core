...
class ConformanceRest extends BackboneElement {
	...
	set resource (new_value) {
		...
		this.__resource = Array.isArray(new_value) ? new_value.map(val => new ConformanceRestResource(val)) : [new ConformanceRestResource(new_value)];
	}
	...
	set interaction (new_value) {
		...
		this.__interaction = Array.isArray(new_value) ? new_value.map(val => new ConformanceRestInteraction(val)) : [new ConformanceRestInteraction(new_value)];
	}
	...
	set operation (new_value) {
		...
		this.__operation = Array.isArray(new_value) ? new_value.map(val => new ConformanceRestOperation(val)) : [new ConformanceRestOperation(new_value)];
	}
