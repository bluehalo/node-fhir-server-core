...
class ConformanceRestSecurity extends BackboneElement {
	...
	set certificate (new_value) {
		...
		this.__certificate = Array.isArray(new_value) ? new_value.map(val => new ConformanceRestSecurityCertificate(val)) : [new ConformanceRestSecurityCertificate(new_value)];
	}
