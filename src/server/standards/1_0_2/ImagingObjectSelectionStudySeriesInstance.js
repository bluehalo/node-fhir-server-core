...
class ImagingObjectSelectionStudySeriesInstance extends BackboneElement {
	...
	set frames (new_value) {
		...
		this.__frames = Array.isArray(new_value) ? new_value.map(val => new ImagingObjectSelectionStudySeriesInstanceFrames(val)) : [new ImagingObjectSelectionStudySeriesInstanceFrames(new_value)];
	}
