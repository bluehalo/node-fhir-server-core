...
class MedicationDispense extends DomainResource {
	...
	set dosageInstruction (new_value) {
		...
		this.__dosageInstruction = Array.isArray(new_value) ? new_value.map(val => new MedicationDispenseDosageInstruction(val)) : [new MedicationDispenseDosageInstruction(new_value)];
	}
