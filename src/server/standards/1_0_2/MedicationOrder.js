...
class MedicationOrder extends DomainResource {
	...
	set dosageInstruction (new_value) {
		...
		this.__dosageInstruction = Array.isArray(new_value) ? new_value.map(val => new MedicationOrderDosageInstruction(val)) : [new MedicationOrderDosageInstruction(new_value)];
	}
