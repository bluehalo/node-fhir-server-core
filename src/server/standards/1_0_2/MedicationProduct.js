...
class MedicationProduct extends BackboneElement {
	...
	set ingredient (new_value) {
		...
		this.__ingredient = Array.isArray(new_value) ? new_value.map(val => new MedicationProductIngredient(val)) : [new MedicationProductIngredient(new_value)];
	}
	...
	set batch (new_value) {
		...
		this.__batch = Array.isArray(new_value) ? new_value.map(val => new MedicationProductBatch(val)) : [new MedicationProductBatch(new_value)];
	}
