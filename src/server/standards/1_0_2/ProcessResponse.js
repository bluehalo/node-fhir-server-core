...
class ProcessResponse extends DomainResource {
	...
	set outcome (new_value) {
		...
		this.__outcome = new Coding(new_value);
	}
	...
	set notes (new_value) {
		const ProcessResponseNotes = require('./ProcessResponseNotes');
		this.__notes = Array.isArray(new_value) ? new_value.map(val => new ProcessResponseNotes(val)) : [new ProcessResponseNotes(new_value)];
	}
	...
	set error (new_value) {
		const Coding = require('./Coding');
		this.__error = Array.isArray(new_value) ? new_value.map(val => new Coding(val)) : [new Coding(new_value)];
	}
