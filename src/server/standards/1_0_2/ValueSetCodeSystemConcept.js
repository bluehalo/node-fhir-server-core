...
class ValueSetCodeSystemConcept extends BackboneElement {
	...
	set designation (new_value) {
		...
		this.__designation = Array.isArray(new_value) ? new_value.map(val => new ValueSetCodeSystemConceptDesignation(val)) : [new ValueSetCodeSystemConceptDesignation(new_value)];
	}
