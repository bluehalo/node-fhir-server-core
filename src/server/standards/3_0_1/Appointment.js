...
class Appointment extends DomainResource {
	...
	toJSON () {
		return Object.assign(super.toJSON(), {
			...
			status: this._status,
			serviceCategory: this._serviceCategory && this._serviceCategory.toJSON(),
			serviceType: this._serviceType && this._serviceType.map(v => v.toJSON()),
			specialty: this._specialty && this._specialty.map(v => v.toJSON()),
			appointmentType: this._appointmentType && this._appointmentType.toJSON(),
			...
			slot: this._slot && this._slot.map(v => v.toJSON()),
			...
			requestedPeriod: this._requestedPeriod && this._requestedPeriod.map(v => v.toJSON())
		});
	}
