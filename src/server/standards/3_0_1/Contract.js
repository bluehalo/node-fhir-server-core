...
class Contract extends DomainResource {
	...
	toJSON () {
		return Object.assign(super.toJSON(), {
			...
			subject: this._subject && this._subject.map(v => v.toJSON()),
			topic: this._topic && this._topic.map(v => v.toJSON()),
			authority: this._authority && this._authority.map(v => v.toJSON()),
			domain: this._domain && this._domain.map(v => v.toJSON()),
			...
			decisionType: this._decisionType && this._decisionType.toJSON(),
			contentDerivative: this._contentDerivative && this._contentDerivative.toJSON(),
			securityLabel: this._securityLabel && this._securityLabel.map(v => v.toJSON()),
			agent: this._agent && this._agent.map(v => v.toJSON()),
			signer: this._signer && this._signer.map(v => v.toJSON()),
			valuedItem: this._valuedItem && this._valuedItem.map(v => v.toJSON()),
			term: this._term && this._term.map(v => v.toJSON()),
			bindingAttachment: this._bindingAttachment && this._bindingAttachment.toJSON(),
			bindingReference: this._bindingReference && this._bindingReference.toJSON(),
			friendly: this._friendly && this._friendly.map(v => v.toJSON()),
			legal: this._legal && this._legal.map(v => v.toJSON()),
