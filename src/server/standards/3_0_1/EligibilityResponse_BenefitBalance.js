...
class EligibilityResponse_BenefitBalance extends BackboneElement {
	...
	set financial ( new_value ) {
		this._financial = Array.isArray(new_value) ? new_value.map(val => new EligibilityResponse_Financial(val)) : [new EligibilityResponse_Financial(new_value)];
	}

	toJSON () {
		return Object.assign(super.toJSON(), {
			category: this._category && this._category.toJSON(),
			subCategory: this._subCategory && this._subCategory.toJSON(),
			excluded: this._excluded,
			name: this._name,
			description: this._description,
			network: this._network && this._network.toJSON(),
			unit: this._unit && this._unit.toJSON(),
			term: this._term && this._term.toJSON(),
			financial: this._financial && this._financial.map(v => v.toJSON())
		});
	}

}

module.exports = EligibilityResponse_BenefitBalance;
