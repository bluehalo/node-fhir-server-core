...
class Encounter extends DomainResource {
	...
	set classHistory(new_value) {
		...
		this.__classHistory = Array.isArray(new_value)
			? new_value.map(val => new EncounterClassHistory(val))
			: [new EncounterClassHistory(new_value)];
	}
	...
	set diagnosis(new_value) {
		...
		this.__diagnosis = Array.isArray(new_value)
			? new_value.map(val => new EncounterDiagnosis(val))
			: [new EncounterDiagnosis(new_value)];
	}
