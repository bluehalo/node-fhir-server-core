...
class ImmunizationRecommendation_Recommendation extends BackboneElement {
	...
	toJSON () {
		return Object.assign(super.toJSON(), {
			...
			targetDisease: this._targetDisease && this._targetDisease.toJSON(),
			doseNumber: this._doseNumber,
			forecastStatus: this._forecastStatus && this._forecastStatus.toJSON(),
			dateCriterion: this._dateCriterion && this._dateCriterion.map(v => v.toJSON()),
			protocol: this._protocol && this._protocol.toJSON(),
			supportingImmunization: this._supportingImmunization && this._supportingImmunization.map(v => v.toJSON()),
			supportingPatientInformation: this._supportingPatientInformation && this._supportingPatientInformation.map(v => v.toJSON())
		});
	}
