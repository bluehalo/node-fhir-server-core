...
class PlanDefinition_RelatedAction extends BackboneElement {
	...
	set relationship ( new_value ) {
		...
		let allowed_values = ['before-start', 'before', 'before-end', 'concurrent-with-start', 'concurrent', 'concurrent-with-end', 'after-start', 'after', 'after-end'];
		if ( new_value && allowed_values.indexOf(new_value) === -1 ) {
			throw new Error(`Expected one of ${allowed_values}, got ${new_value} for field relationship`);
		}
	...
	toJSON () {
		return Object.assign(super.toJSON(), {
			...
			offsetDuration: this._offsetDuration && this._offsetDuration.toJSON(),
			offsetRange: this._offsetRange && this._offsetRange.toJSON()
		});
	}
