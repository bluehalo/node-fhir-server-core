...
class StructureMap extends DomainResource {
	...
	set resourceType ( new_value ) {
		...
		let allowed_values = ['StructureMap'];
	...
	set structure ( new_value ) {
		this._structure = Array.isArray(new_value) ? new_value.map(val => new StructureMap_Structure(val)) : [new StructureMap_Structure(new_value)];
	}
	...
	set import ( new_value ) {
		this._import = Array.isArray(new_value) ? new_value.map(val => val) : [new_value];
	}
	...
	set group ( new_value ) {
		this._group = Array.isArray(new_value) ? new_value.map(val => new StructureMap_Group(val)) : [new StructureMap_Group(new_value)];
	}
	...
	toJSON () {
		return Object.assign(super.toJSON(), {
			...
			structure: this._structure && this._structure.map(v => v.toJSON()),
			import: this._import,
