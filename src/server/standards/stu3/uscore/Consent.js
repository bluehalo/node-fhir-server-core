...
class Consent extends DomainResource {
	...
	toJSON () {
		return Object.assign(super.toJSON(), {
			...
			consentingParty: this.consentingParty.map(v => v.toJSON()),
			actor: this.actor.map(v => v.toJSON()),
			action: this.action.map(v => v.toJSON()),
			organization: this.organization.map(v => v.toJSON()),
			...
			policy: this.policy.map(v => v.toJSON()),
			...
			except: this.except.map(v => v.toJSON())
		});
	}
